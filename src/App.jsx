...
import './App.css'; // Adjust path if using src/styles/
...
export default function App() {
  ...
  <Link 
    ...
    className={`transition-colors duration-200 no-underline ${
      isActive('/posts') ? 'text-icy-blue font-semibold' : 'text-white-smoke hover:text-icy-blue'
    }`}
  >
    Posts
  </Link>
  <Link 
    to="/my-profile" 
    className={`transition-colors duration-200 no-underline ${
      isActive('/my-profile') ? 'text-icy-blue font-semibold' : 'text-white-smoke hover:text-icy-blue'
    }`}
  >
    My Profile
  </Link>
    ...
    Login
      ...
      <main className="flex-1 px-4 py-2 flex items-center justify-center overflow-hidden">
        <Routes>
          ...
          <Route path="*" element={<h2 className="text-xl font-bold text-midnight-violet">404 - Page Not Found</h2>} />
        </Routes>
      </main>
    </div>
  );
